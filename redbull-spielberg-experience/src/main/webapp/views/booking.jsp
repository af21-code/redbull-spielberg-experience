...
<head>
  ...
  <link rel="stylesheet" href="<%=ctx%>/styles/booking.css?v=<%= cssVer %>">
  ...
  <style>
    .no-booking-css body::before{
      ...
      display:block;background:#b33939;color:#fff;padding:8px 12px;font-weight:800;text-align:center;
    }
  </style>
</head>
      ...
      <div class="col-left">
        ...
        <div class="booking-step card" data-step="1">
          <header class="card-header">
              ...
              <h3 class="card-title">Seleziona data e orario</h3>
          </header>

          <div class="card-body">
            <p class="muted section-info"><%= itDesc %></p>

            <!-- era un <form id="dateForm">: ora è un DIV -->
            <div id="dateForm" class="booking-filter">
              <input type="hidden" name="productId" value="<%= productId %>">
              <label class="sr-only">Seleziona una data
                <input type="date"
                       id="date-input"
                       name="date"
              ...
              <button type="button" id="findBtn" class="btn primary">Cerca</button>
            </div>

            <div class="date-section">
              <h4 class="section-subtitle">Prossime date disponibili</h4>
              <div id="avail" class="date-grid" aria-live="polite">Caricamento…</div>
              <p class="section-note">💡 Clicca una data per vedere gli orari.</p>
            </div>

            <fieldset class="section">
              <legend class="sr-only">Seleziona orario</legend>
              <div class="section-head">
                <h3>Orari disponibili per il <span id="selected-date-display"><%= (selectedDate==null?"—":selectedDate.toString()) %></span></h3>
                <small class="muted" id="total-remaining-note">
                  Totale posti: <strong><%= totalRemaining %></strong>
                </small>
              </div>
              ...
              <div class="slot-grid">
                ...
                <%
                    ...
                    boolean firstRequiredSet = false;
                    for (SlotVM s : slotsVm) {
                      boolean isDisabled = s.isSoldOut();
                      boolean setRequired = !isDisabled && !firstRequiredSet;
                      if (setRequired) firstRequiredSet = true;

                      String lbl = s.getLabel()==null? "" : s.getLabel();
                      String timePart = lbl, remPart = "";
                      int dotIdx = lbl.indexOf("·");
                      if (dotIdx > 0) timePart = lbl.substring(0, dotIdx).trim();
                      int postiIdx = lbl.lastIndexOf("posti");
                      if (postiIdx >= 0) {
                        String after = lbl.substring(postiIdx + "posti".length()).trim();
                        StringBuilder num = new StringBuilder();
                        for (int i=0;i<after.length();i++){ char c = after.charAt(i); if (Character.isDigit(c)) num.append(c); else break; }
              ...
              <div class="vehicle-grid">
                <% if (vehiclesVm != null && !vehiclesVm.isEmpty()) {
                  ...
                  <label class="vehicle-card <%= (v.isFeatured() ? "featured" : "") %>">
                    <input type="radio" name="vehicleChoice" value="<%= v.getCode() %>" <%= v.isChecked() ? "checked" : "" %> >
                    <div class="veh-inner">
        ...
        <form class="booking-form" id="addToCartForm" method="post" action="<%=ctx%>/cart/add" novalidate>
          ...
          <input type="hidden" name="eventDate" value="<%= (selectedDate==null? "" : selectedDate.toString()) %>">
          ...
          <div class="card sidebar-card">
            ...
            <div class="sidebar-body">
                ...
                <p class="sidebar-item">
                    ...
                    <span class="value" id="summary-date"><%= (selectedDate==null?"—":selectedDate.toString()) %></span>
                </p>
                <p class="sidebar-item">
                ...
                <p class="sidebar-item">
                    ...
                    <span class="value" id="summary-vehicle">
                        <% if (vehiclesVm != null) {
                            String vLbl = "—";
                            for (VehicleVM v : vehiclesVm) { if (v.isChecked()) { vLbl = v.getLabel(); break; } }
                            out.print(vLbl);
                        } else { out.print("—"); } %>
                    </span>
                </p>
                ...
                <div class="sidebar-cta">
                  <button id="finalSubmitBtn" class="btn primary btn-lg" type="submit" form="addToCartForm" <%= disableAddToCart ? "disabled" : "" %>>
