  ...
  <%@ page import="java.util.*, java.time.LocalDate" %>
    <%@ page import="control.BookingServlet.SlotVM, control.BookingServlet.VehicleVM" %>
      <% String ctx=request.getContextPath(); String cssVer=(String) request.getAttribute("cssVersion"); String
        ...
        request.getAttribute("itShort"); String itDesc=(String) request.getAttribute("itDesc"); String heroImg=(String)
        request.getAttribute("heroImg"); String overlayWall=(String) request.getAttribute("overlayWall"); String
        expCode=(String) request.getAttribute("expCode"); Integer productId=(Integer) request.getAttribute("productId");
        LocalDate selectedDate=(LocalDate) request.getAttribute("selectedDate"); @SuppressWarnings("unchecked")
        List<SlotVM> slotsVm = (List<SlotVM>) request.getAttribute("slotsVm");
          ...
          List<VehicleVM> vehiclesVm = (List<VehicleVM>) request.getAttribute("vehiclesVm");
              ...
              if (slotsVm != null) {
              for (SlotVM s : slotsVm) { if (!s.isSoldOut()) { hasSellableSlot = true; break; } }
              }
              boolean disableAddToCart = (selectedDate==null) || (slotsVm==null) || !hasSellableSlot;

              String defaultVehicleCode = "";
              if (vehiclesVm != null) {
              for (VehicleVM v : vehiclesVm) { if (v.isChecked()) { defaultVehicleCode = v.getCode(); break; } }
              }
              %>
              <!DOCTYPE html>
              <html lang="it">
              ...
              <% } else { %>
                ...
                <section class="booking-shell">
                  <div class="booking-grid">
                    ...
                    <div class="col-left">
                      ...
                      <div class="booking-step card" data-step="1">
                        ...
                        <div class="card-body">
                          ...
                          <div class="date-section">
                            ...
                            <p class="section-note">Tocca una data qui sotto o scegli dal calendario: aggiorniamo gli
                          ...
                          <fieldset class="section">
                            ...
                            <div class="slot-grid">
                              <% if (selectedDate==null) { %>
                                ...
                                <% } else if (slotsVm !=null && !slotsVm.isEmpty()) { boolean firstRequiredSet=false;
                                  for (SlotVM s : slotsVm) { boolean isDisabled=s.isSoldOut(); boolean
                                  setRequired=!isDisabled && !firstRequiredSet; if (setRequired) firstRequiredSet=true;
                                  String lbl=s.getLabel()==null? "" : s.getLabel(); String timePart=lbl, remPart="" ;
                                  int dotIdx=lbl.indexOf("·"); if (dotIdx> 0) timePart = lbl.substring(0,
                                  dotIdx).trim();
                                  int postiIdx = lbl.lastIndexOf("posti");
                                  if (postiIdx >= 0) {
                                  String after = lbl.substring(postiIdx + "posti".length()).trim();
                                  StringBuilder num = new StringBuilder();
                                  for (int i=0;i<after.length();i++){ char c=after.charAt(i); if (Character.isDigit(c))
                                    num.append(c); else break; } remPart=num.toString(); } %>
                                    ...
                                    <% } } else { %>
                                      <div class="empty">Nessuno slot disponibile per questa data. Prova a cambiarla.
                                      </div>
                                      <% } %>
                            </div>
                          </fieldset>
                        ...
                        <div class="card-body">
                          <fieldset class="section">
                            ...
                            <div class="vehicle-grid">
                              <% if (vehiclesVm !=null && !vehiclesVm.isEmpty()) { for (VehicleVM v : vehiclesVm) { %>
                                <label class="vehicle-card <%= (v.isFeatured() ? " featured" : "" ) %>">
                                  <input type="radio" name="vehicleChoice" value="<%= v.getCode() %>" <%=v.isChecked()
                                    ? "checked" : "" %> >
                                  <div class="veh-inner">
                                    <% if (v.isFeatured()) { %>
                      ...
                      <form class="booking-form" id="addToCartForm" method="post" action="<%=ctx%>/cart/add" novalidate>
                        <input type="hidden" name="productId" value="<%= productId %>">
                        ...
                        <div class="booking-step card" data-step="3">
                          ...
                          <div class="card-body">
                            <div class="form-grid two-1">
                              <label>Nome pilota*
                                <input class="input-lg" type="text" name="driverName" placeholder="Es. Max Verstappen"
                                  autocomplete="name" pattern="[A-Za-zÀ-ÿ .'\\-]{2,}"
                                  title="Inserisci un nome valido (min 2 caratteri)" required>
                              </label>
                            ...
                            <div class="form-grid one">
                              <label>Accompagnatore (opzionale)
                                <input class="input-lg" type="text" name="companionName" placeholder="Es. Helmut Marko"
                                  autocomplete="name" pattern="[A-Za-zÀ-ÿ .'\\-]{2,}">
                              </label>
                            </div>
                          </div>
                          ...
                          <div class="sidebar-body">
                            ...
                            <p class="sidebar-item">
                              ...
                              <span class="value" id="summary-vehicle">
                                <% if (vehiclesVm !=null) { String vLbl="—" ; for (VehicleVM v : vehiclesVm) { if
                                  (v.isChecked()) { vLbl=v.getLabel(); break; } } out.print(vLbl); } else {
                                  out.print("—"); } %>
                              </span>
                            </p>

                            <div class="sidebar-cta">
                              <button id="finalSubmitBtn" class="btn primary btn-lg" type="submit" form="addToCartForm"
                ...
                <script>
                  (function () {
                    ...
                    function updateSlotSummary(label) { if (els.summarySlot) els.summarySlot.textContent = label || '—'; }
                    function toggleSubmit(enabled) { if (els.submitBtn) els.submitBtn.disabled = !enabled; }
                    function updateTotals(total) {
                      if (!els.totalNote) return;
                      var tot = Number(total); if (isNaN(tot)) tot = 0;
                    ...
                    function buildFetchInit() {
                      return { method: 'GET', credentials: 'same-origin', headers: { 'Accept': 'application/json' }, cache: 'no-store' };
                    }
                    function parseMaybeJson(res) {
                      var ct = (res.headers.get('content-type') || '').toLowerCase();
                      if (ct.indexOf('application/json') >= 0) { return res.json(); }
                      return res.text().then(function (txt) { return { __html: txt || '', __url: res.url, __status: res.status, __redirected: !!res.redirected }; });
                    }
                    ...
                    function loadDates() {
                      ...
                      try {
                        ...
                        fetch(url, buildFetchInit())
                          .then(function (res) {
                            if (!res.ok) { els.avail.textContent = 'Errore nel caricamento (' + res.status + ').'; return null; }
                          ...
                          .then(function (data) {
                            ...
                            if (!Array.isArray(data.days)) { els.avail.textContent = 'Nessun dato ricevuto.'; return; }

                            var days = data.days.slice();
                            if (!days.length) { els.avail.textContent = 'Nessuna disponibilità nei prossimi giorni.'; return; }
                            ...
                            days.forEach(function (d) {
                              ...
                              var lbl = dt.toLocaleDateString('it-IT', { weekday: 'short', day: '2-digit', month: '2-digit' });
                          ...
                          .catch(function () { els.avail.textContent = 'Errore di rete.'; });
                      } catch (e) {
                    ...
                    function loadSlots(iso) {
                      ...
                      try {
                        ...
                        fetch(url, buildFetchInit())
                          .then(function (res) {
                            if (!res.ok) { els.slotGrid.innerHTML = '<div class="empty">Errore nel caricamento (' + res.status + ').</div>'; return null; }
                            return parseMaybeJson(res);
                          })
                          .then(function (data) {
                            if (!data) return;
                          ...
                          .catch(function () { els.slotGrid.innerHTML = '<div class="empty">Errore di rete.</div>'; });
                      } catch (e) {
