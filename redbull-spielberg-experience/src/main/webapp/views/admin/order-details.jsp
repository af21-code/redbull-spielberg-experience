...
<%!
  ...
  private static List<Map<String,Object>> asListOfMapSO(Object obj) {
    ...
    if (obj instanceof List<?>) {
      for (Object x : (List<?>) obj) if (x instanceof Map<?,?>) out.add((Map<String,Object>) x);
    }
    return out;
  }
%>
...
<head>
  ...
  <title>Admin • Ordine <%= esc(onum) %></title>
      ...
      <div class="col-right">
        ...
        <div class="card" style="margin-top:18px">
          ...
          <form method="post" action="<%=ctx%>/admin/order-action" style="margin-bottom:10px">
            ...
            <input type="text" name="carrier" class="block" placeholder="Corriere (DHL/UPS/FEDEX...)" value="<%= carrier==null? "": esc(carrier) %>" style="margin-bottom:8px">
            <input type="text" name="tracking_code" class="block" placeholder="Codice tracking" value="<%= tracking==null? "": esc(tracking) %>" style="margin-bottom:8px">
