...
<%
  ...
  // Evita cast non-checkati: lavori con List<Map<?,?>>
  Object ordAttr = request.getAttribute("orders");
  List<Map<?,?>> orders;
  if (ordAttr instanceof List<?>) {
    List<?> tmp = (List<?>) ordAttr;
    List<Map<?,?>> safe = new ArrayList<>();
    for (Object x : tmp) if (x instanceof Map<?,?>) safe.add((Map<?,?>) x);
    orders = safe;
  } else {
  ...
  <div class="container">
    ...
    <div class="card filters">
      <form method="get" action="<%=ctx%>/admin/orders" class="row" style="align-items:flex-end">
        ...
        <div class="field" style="min-width:220px">
          ...
          <input type="text" id="q" name="q" value="<%= esc(q) %>" placeholder="Es. mario.rossi@email.com">
        </div>
        <div class="field">
        ...
        <div class="right">
          <a class="btn" href="<%=ctx%>/admin/orders?<%= esc(baseQS) %>&amp;export=csv">Esporta CSV</a>
        </div>
      </form>
    </div>
    ...
    <div class="card">
      <table class="table">
        ...
        <%
          for (Map<?,?> r : orders) {
            ...
            BigDecimal tot = (r.get("total_amount") instanceof BigDecimal) ? (BigDecimal) r.get("total_amount") : BigDecimal.ZERO;
        ...
        %>
          <tr>
            ...
            <td style="max-width:280px"><%= esc(cust) %></td>
            <td>€ <%= tot %></td>
      ...
      <div class="pagination">
        <% if (pageNo > 1) { %>
          <a href="<%=ctx%>/admin/orders?<%= esc(baseQS) %>&amp;page=<%= (pageNo-1) %>">« Prec</a>
        ...
        <% for (int i = 1; i <= pages; i++) { %>
          ...
          <% } else { %>
            <a href="<%=ctx%>/admin/orders?<%= esc(baseQS) %>&amp;page=<%= i %>"><%= i %></a>
          <% } %>
        <% } %>

        <% if (pageNo < pages) { %>
          <a href="<%=ctx%>/admin/orders?<%= esc(baseQS) %>&amp;page=<%= (pageNo+1) %>">Succ »</a>
