<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
        ...
        <%@ page import="model.Product, model.Product.ProductType, model.Category" %>
            <%@ page import="java.text.DecimalFormat, java.text.DecimalFormatSymbols, java.util.Locale" %>
                <%! private static String esc(Object o) { if (o==null) return "" ; String
                    ...
                    %>
                    <% String ctx=request.getContextPath(); String csrf=(String) session.getAttribute("csrfToken");
    ...
    for (Object x : (List<?>) prodAttr) if (x instanceof Product) products.add((Product) x);
                            ...
                            // Get categories from servlet
                            List<Category> categories = new ArrayList<>();
                                    Object catAttr = request.getAttribute("categories");
                                    if (catAttr instanceof List
                                    <?>) {
                            for (Object x : (List<?>) catAttr) if (x instanceof Category) categories.add((Category) x);
                                    }
                                        ...
                                        <section class="shop-container">
                                                    ...
                                                    <select name="category" onchange="this.form.submit()">
                                                        ...
                                                        <% for (Category c : categories) { %>
                                                            <option value="<%= c.getCategoryId() %>"
                                                                ...
                                                                selectedCategory.equals(c.getCategoryId())) ? "selected"
                                                                : "" %>>
                                                                <%= esc(c.getName()) %>
                                                            </option>
                                                            <% } %>
                                            ...
                                            <div class="product-grid">
                                                <% if (products.isEmpty()) { %>
                                                    ...
                                                    <% } else { for (Product p : products) { String
                                                                ...
                                                                <div class="meta">
                                                                    ...
                                                                    <% if (isMerch && hasStock) { %>
                                                                        <span
                                                                            ...
                                                                            <%= p.getStockQuantity()> 0 ? "Disponibile"
                                                                                :
                                                                                "Esaurito" %>
                                                                        </span>
                                                                        <% } %>
