...
public class UserAdminServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        HttpSession session = req.getSession(false);
        if (!SecurityUtils.isAdmin(session)) {
            // Se non loggato -> login, se loggato ma non admin -> 403
            // Per semplicità qui faccio redirect a login se non admin, oppure 403.
            // SecurityUtils check failure means either not logged in OR not admin.
            // If session is null or authUser is null -> redirect login logic?
            // Existing logic separated null check (login) vs admin check (403).
            // SecurityUtils returns false for both.
            // I'll stick to redirecting to login page for now as it's safe.
            resp.sendRedirect(req.getContextPath() + "/views/login.jsp");
            return;
        }
